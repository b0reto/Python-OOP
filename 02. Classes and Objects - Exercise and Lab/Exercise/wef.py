class PizzaDelivery:
    ...
    def make_order(self):
        if not self.ordered:
            ...
            a = [f'{key}: {value}' for key, value in self.ingredients.items()]
            # for el in a:
            #     print(el, end=', ')
            return f"You've ordered pizza {self.name} prepared with {[print(el, end=', ') for el in a]} and the price will be {self.price}lv."
